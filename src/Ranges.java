...
public class Ranges implements Iterable<Interval> {
  ...
  private void replaceIfOverlapping(Interval toRemove, ListIterator<Interval> iter, Interval current) {
    if (current.overlaps(toRemove)) {
      ...
      iter.remove();
      replaceOverlappingInterval(iter, lowerPart);
      replaceOverlappingInterval(iter, upperPart);
    }
  }

  private void replaceOverlappingInterval(ListIterator<Interval> iter, Interval part) {
    if (part.width() > 0.0) {
      iter.add(part);
    }
  }
